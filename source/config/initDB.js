...
const seedDB = async () => {
    try {
        ...
        // Insert events after locations
        for (const event of eventData) {
            await pool.query(newEventQuery, [event.location_id, event.name, event.description, event.date, event.time, event.price, event.expired, event.datePosted]);
